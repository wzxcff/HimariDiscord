...
log_channel_id = 1205606154072358942
welcome_channel_id = 923997595888656404
temporary_channels = []
...
@commands.has_any_role('Admin', 'Insider', 'chatmoder')
@bot.command(name="trust", help="trust *@user*")
async def trust_this_user(ctx, member: discord.Member, user_name="empty"):
    await member.add_roles(discord.utils.get(ctx.guild.roles, name="Trusted"))
    await ctx.send(f"User <@{member.id}> is now trusted!")


@commands.has_any_role('Admin', 'Insider', 'chatmoder')
@bot.command(name="untrust", help="untrust *@user*")
async def untrust_this_user(ctx, member: discord.Member):
    await member.remove_roles(discord.utils.get(ctx.guild.roles, name="Trusted"))
    await ctx.send(f"User <@{member.id}> from now is not trusted!")


@bot.command(name='create-channel', help="create-channel *name*")
@commands.has_any_role('Admin', 'Insider')
...
@bot.command(name="kick", help="kick *@user*", pass_context=True)
...
async def on_member_join(member):
    ...
    embed_message = discord.Embed(title=f"**{member.display_name}** just connected!", description="\"-trust\" to trust them :white_heart:")
